...
// --- B/W MID (half/scrub) : 5 x 43, command-prefixed --- [CrossMosa bench 候選一]
// 直譯自 UC8253 X3 的 _half scrub bank（同一片玻璃上實證的 50 幀低閃清潔檔）：
// 同為 UltraChip 7 組 × [VS TP0 TP1 TP2 TP3 RP] 結構，狀態驅動（WW==BW、WB==BB
// —— 不看舊態、把每個像素朝目標態刷兩輪），無全黑反轉段 = 低閃。
// ⚠️ 電軌不同（8279 init: PWR 43 00 78 78 17 / PLL 0F vs 8253 各自設定），
// 對比與殘影表現需實機目測 —— 這是 bench 候選，不是保證。LUT 是揮發的：
// 效果不好重刷韌體即復原，不影響開機路徑。
// bench 候選二（Chihao 第一輪目測：「閃爍可接受／清潔好一點／對比想再黑一點」）：
// 加第三組「純黑收尾」—— 只有 wb/bb 在 phase0 驅動（VS=0x40，MSB-first 的 01）4 幀，
// ww/bw/vcom 同時段靜止（0x00）→ 白區零擾動、黑字多吃 4 幀墨、不增加閃爍。
// 五張表計時一致律：TP 欄全表相同（[4,0,0,0] rp=0），差異只在 VS 位元。總幀 50→54。
inline const uint8_t kUc8279X3_BwMid[5][43] = {
    {0x20, 0x00, 0x06, 0x01, 0x06, 0x06, 0x01, 0x00, 0x04, 0x01, 0x01, 0x00, 0x01, 0x00, 0x04, 0x00, 0x00, 0x00, 0x00},
    {0x21, 0xAA, 0x06, 0x01, 0x06, 0x06, 0x01, 0xA0, 0x04, 0x01, 0x01, 0x00, 0x01, 0x00, 0x04, 0x00, 0x00, 0x00, 0x00},
    {0x22, 0xAA, 0x06, 0x01, 0x06, 0x06, 0x01, 0xA0, 0x04, 0x01, 0x01, 0x00, 0x01, 0x00, 0x04, 0x00, 0x00, 0x00, 0x00},
    {0x23, 0x55, 0x06, 0x01, 0x06, 0x06, 0x01, 0x50, 0x04, 0x01, 0x01, 0x00, 0x01, 0x40, 0x04, 0x00, 0x00, 0x00, 0x00},
    {0x24, 0x55, 0x06, 0x01, 0x06, 0x06, 0x01, 0x50, 0x04, 0x01, 0x01, 0x00, 0x01, 0x40, 0x04, 0x00, 0x00, 0x00, 0x00},
};
